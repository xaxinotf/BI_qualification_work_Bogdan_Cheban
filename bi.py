...
requests_merged = requests_df.merge(military_personnel_df, left_on="military_personnel_id", right_on="mp_id",
                                    how="left")
...
if __name__ == '__main__':
    ...
    # Виконуємо аналіз зборів і зберігаємо результати (файли створюються одразу)
    levy_analysis_instance.run()
    ...
    app.layout = html.Div(style={"display": "flex"}, children=[
        html.Div(
            style={"backgroundColor": "#fff", "padding": "20px", "borderRight": "2px solid #ddd", "height": "100vh",
    ...
    def render_tab_content(tab_value):
        if tab_value == "tab-analytics":
            return html.Div([
                html.Div(style={"padding": "10px", "backgroundColor": "#fff", "border": "2px solid #ddd",
                                "marginBottom": "20px"},
                         children=[
                             ...
                             dcc.Dropdown(
                                 ...
                                 options=[{"label": role, "value": role} for role in
                                          sorted(users_df["user_role"].unique())],
                             ...
                             dcc.RangeSlider(
                                 ...
                                 value=[int(users_df["duration_on_platform"].min()),
                                        int(users_df["duration_on_platform"].max())],
            ...
            return html.Div(
                style={"padding": "20px", "backgroundColor": "#fff", "border": "2px solid #ddd", "marginTop": "20px"},
                children=[
                    ...
                    html.Div(id="drill-down", style={"padding": "20px", "marginTop": "20px", "border": "2px solid #ddd",
                                                     "backgroundColor": "#fff"})
                ])
            ...
            return html.Div(
                ...
                children=[
                    ...
                    dcc.Dropdown(
                        id="cluster-filter",
                        options=[{"label": f"Кластер {i}", "value": int(i)} for i in
                                 sorted(donation_stats["cluster"].unique())],
            ...
            return html.Div(
                ...
                children=[
                    ...
                    html.Div(id="kpi-div-updated",
                             children=[html.H2("Ключові показники (KPI)", style={"color": "#333"}), kpi_div]),
                    html.Div([html.H2("Історія KPI", style={"color": "#333"}),
            ...
            return html.Div(
                ...
                children=[
                    ...
                    html.H3("Тематичний аналіз відгуків (топ-10 ключових слів)",
                            style={"color": "#333", "textAlign": "center", "marginTop": "20px"}),
                    dcc.Graph(id="volunteer-topwords", figure=fig_vol_topwords)
                ])
        elif tab_value == "tab-levy":
            from analyze2 import LevyAnalysis
            la = LevyAnalysis(data_dir="data")
            # Обчислюємо аналіз зборів та отримуємо три графіки:
            # 1. Scatter-графік: ефективність зборів vs. кількість звітів
            # 2. Часовий тренд (з прогнозом) накопичення коштів
            # 3. Матриця кореляції
    ...
    def drill_down(nodeData):
        ...
        if node_label == "User":
            ...
            orders_by_day = liqpay_orders_df.groupby(liqpay_orders_df["create_date"].dt.date)[
    ...
    def update_kpi(n_clicks, total_target, avg_target, unique_target, percent_target, max_target, avg_reg_target,
                   corr_target, kpi_content):
        if n_clicks == 0:
            return "", kpi_content
        new_targets = {
        ...
        if not messages:
            messages = html.P("Всі KPI відповідають встановленим цілям.",
    ...
    def update_product_chart(metric, topic_filter):
        ...
        y_label = {"request_count": "Кількість заявок", "total_amount": "Загальна сума заявок",
                   "average_amount": "Середня сума заявки"}.get(metric, metric)
